...
async function VideoServers() {//SERVERS
  const Server_Search = await fetch('/api/server/' + episodeId);
...
async function AnimeDetail_Prev_Next() {//Anime Details and PREV NEXT
  let num = 0;
  if (isNaN(AnimeId)) {
    const info = await fetch('/api/search/' + episodeName + "?page=1&perPage=1");
  ...
  if (num === episodeNumber) {
    prev.href = '/db/watch-delete-previous/' + episodeName + '?id=' + `${episodeNumber - 1}` + "&num=" + NumberOfAnime + "&img=" + AnimeImg;
    // color: slategray;
    ...
    next.href = '/db/watch-delete-previous/' + episodeName + '?id=' + `${episodeNumber + 1}` + "&num=" + NumberOfAnime + "&img=" + AnimeImg;
  }
  else {
    prev.href = '/db/watch-delete-previous/' + episodeName + '?id=' + `${episodeNumber - 1}` + "&num=" + NumberOfAnime + "&img=" + AnimeImg;
    next.href = '/db/watch-delete-previous/' + episodeName + '?id=' + `${episodeNumber + 1}` + "&num=" + NumberOfAnime + "&img=" + AnimeImg;
