...
let Popular_url = '/api/trending?' + pagename + "&perPage=25";
...
document.getElementById('search-Anilist').addEventListener('click', async (e) => {
    let Advanced_Url = '/api/advanced-search?' + Advanced_Search_link.textContent;
...
pages_li.forEach((element) => {
    ...
    element.addEventListener('click', async () => {
        if (type === 0) {
            window.location.href = `/Popular-Anime?pageNum=${element.innerHTML}`
        }
        else {
            users.forEach(user => {
                user.element.remove();
            })
            let Advanced_Url = `/api/advanced-search?${Advanced_Search_link.textContent}&page=${element.innerHTML}`;
...
First_Page.addEventListener('click', async () => {
    if (type === 0) {
        ...
        let Advanced_Url = `/api/advanced-search?${Advanced_Search_link.textContent}&page=1`;
        const response_for_Advance = await fetch(Advanced_Url);
        animeRecents = await response_for_Advance.json();
        users = card_generator(animeRecents, users);
        let x = 1;
...
Last_Page.addEventListener('click', async () => {
    if (type === 0) {
        ...
        let Advanced_Url = `/api/advanced-search?${Advanced_Search_link.textContent}&page=30`;
        const response_for_Advance = await fetch(Advanced_Url);
        animeRecents = await response_for_Advance.json();
        users = card_generator(animeRecents, users);
        let x = 26;
