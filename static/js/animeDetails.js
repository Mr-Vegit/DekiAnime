...
console.log(animeId);
const response = await fetch('/api/info/' + animeId + "?dub=" + dub);
...
AnimeDetails_Data.episodes.forEach(user => {
    ...
    AnimeLink.href = '/db/watch-delete-previous/' + y + '?' + "id=" + user.number + "&num=" + AnimeDetails_Data.id+"&img="+user.image;
...
document.getElementById('bookmark-link').addEventListener('click', (e) => {
    ...
    window.location.href = "/db/bookmark-save-delete?animeId="+animeId+"&animeTitle="+AnimeDetails_Data.title.romaji+"&img="+AnimeDetails_Data.image;
    }
});
